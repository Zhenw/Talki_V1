...
bool bonding_data_purging_all_keys_pressed __RETAINED;      //
 ...
 * @param[in] pressed
 *
 * @return  0, if the key is ignored due to ghosting
 *          1, if the keycode has been accepted (passed debouncing, deghosting).
...
static inline int record_key(const uint16 output, const uint16 input, const int pressed)
{
    ...
    if (pressed)
    {
        if (kbd_ir_keymap[0][output][input] != 0xFFFF && !app_rc5_is_busy())
            app_rc5_send_command(kbd_ir_keymap[0][output][input]);
    }
    else
    {
        app_rc5_stop_sending();
    }   
#endif //USE_IR
...
void process_key_combinations(void)
{
    ...
    for (i = 0; i < KBD_NR_OUTPUTS; i++)
        ...
        {
            int release_cond = ((~key_comb[j].scan_status) & ((1 << KBD_NR_INPUTS) - 1));
            
            if ( ((key_comb[j].trigger_on_release == false) && ( (kbd_scandata[i] | key_comb[j].scan_status) == key_comb[j].scan_status ) ) ||
                 ((key_comb[j].trigger_on_release == true) &&  ( (kbd_scandata[i] & release_cond) == release_cond  ))) 
                {
                    // execute action
                    ...
                    // The key combination for clearing bonding data and disconnecting is detected here (ALL COMBINATION KEYS PRESSED)
                    if ((key_comb[j].action == 0x1111) && (key_comb[j].row_number == i))
                    {
                        bonding_data_purging_all_keys_pressed = true;   
                    } 
                    // The key combination for clearing bonding data and disconnecting is detected here (ALL COMBINATION KEYS RELEASED AFTER BEING ALL PRESSED)
                    if ((key_comb[j].action == 0x2222) && (key_comb[j].row_number == i))
                    {
                        if (bonding_data_purging_all_keys_pressed)
                        {
                            bonding_data_purging_all_keys_pressed = false;
                            app_alt_pair_clear_all_bond_data();     
                            reset_bonding_request = true;													
                        }           
                    }                
...
// Add support for usages where on and off are controlled by two separate buttons
// e.g. teletext_on and teletext_off buttons                    
#ifdef SUPPORT_ON_AND_OFF_FROM_SEPARATE_BUTTONS
                    if (pressed)
                    {        
                        if ( (byte & 0x08) == 0)    //msb of the "byte" nibble is 0
                        {  
                            // normal case
                        ...
                        {
                            // distinct buttons for ON and OFF of the usage
                            byte &= 0x07;
                            if (code%2)
                            {
                                //OFF
                                code &= ~0x01;
                                pReportInfo->pBuf[byte] |= 0x03<<code;
                            }
                            else
                            {
                                //ON
                                pReportInfo->pBuf[byte] &= ~(0x03 << code);
                                pReportInfo->pBuf[byte] |= 0x01<<code;                                   
                            }
                        }
                    }                  
                    else
                    {
                        ...
                        {
                            // distinct buttons for ON and OFF of the usage
                            pReportInfo->pBuf[byte & 0x07] &= ~(0x03 << (code &= ~0x01));                       
                        }                                                         
                    }                   
#else
                    if (pressed)
                        ...
                        pReportInfo->pBuf[byte] &= ~mask;
#endif //SUPPORT_ON_AND_OFF_FROM_SEPARATE_BUTTONS
