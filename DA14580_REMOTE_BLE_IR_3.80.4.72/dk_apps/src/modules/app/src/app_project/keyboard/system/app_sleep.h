...
static void delayed_start_proc(void)
{
    if (HAS_DELAYED_WAKEUP)
        if ((trigger_kbd_delayed_start_st == PRESSED_TRIGGER ||
             trigger_kbd_delayed_start_st == RELEASED_TRIGGER) && !ble_is_woken_up)
        {
            ...
            struct hid_msg* msg = KE_MSG_ALLOC(APP_HID_MSG, TASK_APP, TASK_APP, hid_msg);
 ...
 ****************************************************************************************
 */
__INLINE bool app_asynch_trm(void)
{
	bool ret = false;

	do {
        // Synchronize with the BLE here!
        // The time window of requesting a packet trm at the upcoming anchor point is
        // from the CSCNT event until the FINEGTIM event.
        // If you pass the FINEGTIM event then the packet will be sent at the next
        // anchor point!
        //
        ...
        if (!ke_event_get(KE_EVENT_KE_MESSAGE))
            // Since pkt reqs can be silently discarded if no Tx bufs are available,
            // check first!
            if (kbd_trm_list && app_kbd_check_conn_status() &&
                l2cm_get_nb_buffer_available())
                if (app_kbd_send_key_report())
                {
                    // One HID report is removed from the trm list.
                    // Check if other HID reports are to be prepared because of unread
                    // data in the keycode_buffer now that the free list is not NULL.
                    app_kbd_prepare_keyreports();
 ...
 ****************************************************************************************
 */
__INLINE bool app_asynch_proc(void)
{
	bool ret = false;
 ...
 ****************************************************************************************
 */
__INLINE void app_asynch_sleep_proc(void)
{
    ...
    if (HAS_KEYBOARD_MEASURE_EXT_SLP)
        if ( (user_extended_sleep) && (current_scan_state == KEY_SCAN_IDLE))
        {
            ...
            // already sleeping, will wake up eventually. wait until this happens.
            if (GetBits16(SYS_STAT_REG, RAD_IS_DOWN))
                return;
                
            if (rwip_prevent_sleep_get())
                return;
 ...
 ****************************************************************************************
 */
__INLINE void app_sleep_prepare_proc(sleep_mode_t *sleep_mode)
{
 ...
 ****************************************************************************************
 */
__INLINE void app_sleep_entry_proc(sleep_mode_t *sleep_mode)
{
 ...
 ****************************************************************************************
 */
__INLINE void app_sleep_exit_proc(sleep_mode_t sleep_mode)
{
    // Restore clock
